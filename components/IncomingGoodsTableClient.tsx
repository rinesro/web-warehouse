...
interface IncomingGoodsTableClientProps {
  data: any[];
  totalPages: number;
  totalItems: number;
  currentPage: number;
...
const IncomingGoodsTableClient = ({
  data,
  totalPages,
  totalItems,
  currentPage,
}: IncomingGoodsTableClientProps) => {
  const columns = useMemo<Column<any>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (item) => (
          <span>
            {item.jumlah_barang} {item.data_barang.satuan_barang}
          </span>
        ),
      },
      {
...
export default IncomingGoodsTableClient;
